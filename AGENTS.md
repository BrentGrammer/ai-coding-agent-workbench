...
- The `gh-axi` and `npm-axi` tools are installed only when the sandbox was started with `--gh-axi` or `--npm-axi`. If they are missing, say so instead of working around them.
